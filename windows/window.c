...
/* rutty: */
#ifdef rutty
    script_init(&scriptdata, &cfg);
    if(cfg.script_mode == SCRIPT_PLAY && (filename_to_str(&cfg.script_filename)[0]!='\0'))
      script_sendfile(&scriptdata, &cfg.script_filename);
	else if(cfg.script_mode == SCRIPT_RECORD && (filename_to_str(&cfg.script_filename)[0]!='\0'))
      script_record(&scriptdata, &cfg.script_filename);
#endif  /* rutty */
...
/* rutty: */
	  ...
	  case IDM_SCRIPTSEND:
        ...
        {
		  script_init(&scriptdata, &cfg);	
    	  script_filename = filename_from_str(scriptfilename);
          script_sendfile(&scriptdata, &script_filename);
        }
	    break;
#endif  /* rutty */        
	  case IDM_CLRSB:
...
{
    script_remote(&scriptdata, data, len);
    return term_data(term, is_stderr, data, len);
}
#else
int from_backend(void *frontend, int is_stderr, const char *data, int len)
